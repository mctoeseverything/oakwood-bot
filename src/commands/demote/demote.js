...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('demote')
    .setDescription('Demote a Roblox user down by one rank')
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages)
    .addStringOption(opt =>
      opt.setName('roblox_username')
        .setDescription('The Roblox username to demote')
        .setRequired(true))
    .addStringOption(opt =>
      opt.setName('reason')
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      console.error('[Demote] Roblox username lookup error:', err.message);
...
if (rankban) {
  ...
  return interaction.editReply({ components: [container], flags: (1 << 15) });
    ...
    if (oldRole && oldRole.rank >= executorRank) {
      return errorReply(interaction, `You cannot demote **@${robloxUser.name}** — their current rank (**${oldRole.rank} — ${oldRole.name}**) is equal to or higher than yours (**${executorRank}**).`);
    }

    if (currentIndex <= 0) {
      return errorReply(interaction, `**@${robloxUser.name}** is already at the lowest rank or their rank could not be determined.`);
    }
